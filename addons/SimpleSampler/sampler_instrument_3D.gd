...
extends Sampler3D
class_name SamplerInstrument3D
...
var samplers : Array[Sampler3D] = []
var next_available := 0
var last_sampler_used: Sampler3D
...
func play_note(note: String, octave: int = 4, velocity: int = 5):
	...
	var sampler: Sampler3D = samplers[next_available]
...
func chord_glide(note: String, octave: int = 4, duration: float = 0.1):
	...
	var ordered_samplers: Array[Sampler3D] = samplers.duplicate()
...
func _can_glide(s: Sampler3D) -> bool:
	return s.playing && !s.in_release && !(s.glissando != null && s.glissando.is_running())

func _build_sampler() -> Sampler3D:
	var sampler := Sampler3D.new()
	...
	_copy_player3D_properties(self, sampler)
