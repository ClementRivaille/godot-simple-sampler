...
extends Sampler2D
class_name SamplerInstrument2D
...
var samplers : Array[Sampler2D] = []
var next_available := 0
var last_sampler_used: Sampler2D
...
func play_note(note: String, octave: int = 4, velocity: int = 5):
	...
	var sampler: Sampler2D = samplers[next_available]
...
func chord_glide(note: String, octave: int = 4, duration: float = 0.1):
	...
	var ordered_samplers: Array[Sampler2D] = samplers.duplicate()
...
func _build_sampler() -> Sampler2D:
	...
	sampler.bus = bus
	_copy_player2D_properties(self, sampler)
